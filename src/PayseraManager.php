...
class PayseraManager {
     ...
     * @return array
     */
    public static function getPaymentMethods($country = null){
        ...
        $payment_methods_all = $country_payment_methods_info->getPaymentMethods();
		
		foreach($payment_methods_all as $key => $method){
			$tmp = [];
			...
			$result['country_payments'][$key] = $tmp;
		}
        return $result;
    }
    ...
    public static function makePayment($order_id, $amount, $options = []){
        try {
            $payment_data = [
				...
				'callbackurl'	=> config('paysera.routes_names.callback'),
				'accepturl'		=> config('paysera.routes_names.accept'),
				'cancelurl'		=> config('paysera.routes_names.cancel')
            ];
